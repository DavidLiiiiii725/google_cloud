...
@app.get("/api/transport/world")
def world_state():
    """Active world event (storm or not). Used by the unified UI to show storm status."""
    storm = db.world_events().find_one(
        {"scenario_id": "default", "type": "storm", "status": "active"})
    return {"storm": storm}


@app.post("/api/transport/world/clear")
async def clear_storm():
    """Resolve the active storm and reopen blocked roads, then re-plan so routes recover."""
    import time as _t
    storm = db.world_events().find_one(
        {"scenario_id": "default", "type": "storm", "status": "active"})
    if not storm:
        return {"ok": True, "cleared": False}
    db.world_events().update_one({"_id": storm["_id"]},
                                 {"$set": {"status": "resolved", "updated_at": _t.time()}})
    for fid in storm.get("effects", {}).get("roads_blocked", []) or []:
        db.farms().update_one({"_id": fid},
                              {"$set": {"access": "open", "yield_status": "normal",
                                        "updated_at": _t.time(), "updated_by": "orchestrator"}})
    events.emit("INFO", f"Storm {storm['_id']} cleared — re-planning")
    plan = await pipeline.build_plan(trigger="storm_cleared")
    return {"ok": True, "cleared": True, "event_id": storm["_id"],
            "plan_id": plan["_id"]}
